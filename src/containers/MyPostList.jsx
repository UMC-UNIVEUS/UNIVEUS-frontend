	...
	return (
		<div className="PostContainer">
			<div className="mainpagebuttons">
				{isClickedOrderButton ? (
					<div>
						<div className="popularorderbutton" onClick={clickPopularOrderButton}>
							인기순
						</div>
						<div className="neworderbutton" onClick={clickNewOrderButton}>
							최신순
						</div>
					</div>
				) : (
					<button className="mainpagebutton orderbutton" onClick={clickOrderButton}>
						{orderOption}
					</button>
				)}
			...
			<div className="container">
				{currentPosts && dummyitems.length > 0 ? (
					currentPosts.map((it) => <MeetingCard {...it} />)
				) : (
					<div>게시물이 없습니다</div>
				)}
			</div>
